...
always @(posedge rst or posedge ts_ack_clr or posedge rtc_timer_clk) begin
  //Si hay un Reset global, iniciamos limpio a 0
  if (rst) begin
    ts_ack <= 1'b0;
  end
  // lógica original de limpieza y seteo
  else if (ts_ack_clr) begin
    ts_ack <= 1'b0;
  end
  else if (ts_req_d2 & !ts_req_d3) begin
    ts_ack <= 1'b1;
  end
end
...
wire q_wr_en = ptp_found && int_eop_d1;                       // Habilitamos la escritura en FIFO
